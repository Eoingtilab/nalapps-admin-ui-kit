...
def license_class(profile: dict) -> str:
    ...
    option_prefix = profile["slug"].replace("-", "_")
    return f'''<?php
/**
...
def update_manager_class(profile: dict) -> str:
    ...
    return f'''<?php
/**
 * EDD hybrid updater: WordPress Plugins screen plus an internal update page.
 *
 * @package {ns}
 */
...
\t\techo '<tr><th>' . esc_html( 'Current version' ) . '</th><td>' . esc_html( {prefix}_VERSION ) . '</td></tr>';
\t\techo '<tr><th>' . esc_html( 'Latest version' ) . '</th><td>' . esc_html( $latest ) . '</td></tr>';
\t\techo '<tr><th>' . esc_html( 'License status' ) . '</th><td>' . esc_html( License::status() ) . '</td></tr>';
\t\techo '<tr><th>' . esc_html( 'Last checked' ) . '</th><td>' . esc_html( $last_checked ?: '-' ) . '</td></tr>';
\t\techo '</tbody></table>';
\t\tif ( $error ) {{
...
\t\t}}
\t\tif ( ! empty( $data['error'] ) ) {{
\t\t\t$message = ! empty( $data['msg'] ) ? sanitize_text_field( (string) $data['msg'] ) : 'Update information is unavailable.';
...
\t\treturn is_array( $info ) && ! empty( $info['new_version'] ) && version_compare( (string) $info['new_version'], {prefix}_VERSION, '>' );
\t}}
